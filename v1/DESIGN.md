...
- **`.hbedit/sidecar/<cardId>.json`** — 上次 pull 的 ProseMirror JSON。
  它有兩個用途:(a) push 時作為 block-ID 移植的基準(見 §4 push);
  (b) 衝突時作為三方合併的 base。
- **`.hbedit/state.json`** — 每張卡的同步狀態。除了判斷哪邊變了,還存
  上次同步的 tag 集合(§8.5 的 3-way tag 合併基準)與 managed 欄位基準
  (§8.6 防呆)。vault root = 最近的含 `.hbedit/` 的祖先目錄(如同 git
  找 `.git/`,見 §8.2)。
...
| 欄位 | 意義 | 寫入方 |
|---|---|---|
| `schemaVersion` | frontmatter schema 版本(目前 `1`);供日後遷移 | hbedit |
| `cardId` | Heptabase 卡片 UUID;卡片的唯一身分 | hbedit |
| `type` | `note`(v1 之後:`journal`) | hbedit |
| `tags` | 卡片的 tag 名稱清單 | 雙向(3-way 合併,見 §8.5) |
| `contentMd5` | 上次 pull 時 Heptabase 回傳的 md5;樂觀鎖用 | hbedit |
| `syncedAt` | 上次成功同步的時間 | hbedit |
...
| 需 desktop app 開著 | app 關閉時所有 `hb` 指令失敗;`hb doctor` 會先回 `app-not-running` 擋下 | §9.4 |
...
2. **`sync.py`** — `pull()` / `push()`,組合既有的 `pm2md` + `transplant`
   + `frontmatter`。
3. **`hb` CLI** — `hb init` / `hb pull` / `hb push` / `hb sync` / `hb status`
   / `hb trash` / `hb tags`。
...
hbedit 的定位一句話講清楚:**它是官方 `heptabase` CLI 缺的那個「編輯
既有卡」動詞。**

官方 CLI 對既有卡片是 append-only —— 能 `note create`、`note append`,但
`note save` 要的是內部 ProseMirror JSON,無法從純文字改卡片中段。hbedit
...
| **改寫 / 重構 / 批次編輯既有卡** | **hbedit** |

三種典型情境:

1. **單卡整理** —— 「把這張卡的標題層級與段落順序重排」。`hb pull` → 用一般
   檔案工具改 `.md` → `hb push`。
2. **跨卡批次** —— 「統一所有 LeetCode 卡的免責聲明格式」。批次 `hb pull` →
   `rg` + 編輯 → `hb sync` 整批推回。
3. **本地長期工作** —— pull 一批卡進 vault,數天內用 agent / 編輯器 / 手改,
   `hb sync` 對帳。vault 是 git repo,筆記從此有 diff 歷史。

對比直接用官方 CLI,優勢五點:

1. **能力缺口** —— 官方 CLI 改不了既有卡中段;這是 hbedit 存在的唯一理由。
...
**呼叫模型** —— hbedit 打包成 skill(`v1/skill/SKILL.md`)。使用者用自然
語言描述需求,Claude 比對 skill description 自動啟動。冷啟動的真實流程是
官方 CLI 與 hbedit 合作:先用官方 `card list -q` 把卡片名稱解析成
cardId,再 `hb pull` 進 vault 編輯。

### 8.2 vault 模型與冷啟動

- **每專案一個 vault** —— vault 不是全域單一,而是綁在專案 / 工作目錄。
- **vault 探測** —— skill 啟動時從 cwd 向上尋找含 `.hbedit/` 的目錄
...
1. **工作單位** —— vault 工作區為主(`.hbedit/state.json` + sidecar),
   同時支援單卡快速操作。`hb pull <cardId>` / `hb push <file>` 為單卡;
   `hb sync` 對整個 vault 批次對帳。
2. **檔案生命週期** —— 無 `cardId` 的檔 = push 時 `note create` 新卡;有
   `cardId` = transplant 更新。**刪檔 = 只取消追蹤**:`hb sync` 偵測
   「state 中曾追蹤、本地檔案已消失」的卡 → 移除 state 條目 + 清 sidecar,
   **Heptabase 卡片不動**。要真正 trash 卡片得用明確指令
   `hb trash <cardId>`(軟刪除,可復原)。
...
1. **push 確認分層** —— transplant 流程中只有最後 `note save` 不可逆,
   ...
   - 明確的 `hb push <file>`(使用者剛指名此檔)→ 直接執行,事後回報
     transplant 摘要。
   - `hb sync` 批次 → 先列出每個 dirty 檔 + 變更摘要,**整批確認一次**
     再寫入。
...
1. **新卡 tag** —— 單卡 `hb push` 不主動問,完全照 frontmatter `tags:`
   (空白就建無 tag 卡)。`hb sync` 批次:摘要列出無 tag 的新卡清單;
   sync 跑完後給**一次性 opt-in 提示**「N 張新卡無 tag,現在掛嗎?」,
   同意才逐卡簡短詢問,永不擋 sync。
2. **建新 tag 的防呆** —— push 要建一個現有 tag 清單裡沒有的 tag 時,先
   與現有 tag 做模糊比對:有相近者(疑似 typo,如 `hbedit` vs
   `Hbedit`)→ 停下來問,讓使用者選用既有或確認建新;完全無相近者
   (真新 tag)→ 直接建並回報。
3. **tag 同步:3-way 合併** —— `contentMd5` 樂觀鎖只管內文、不管 tag,
   ...
   集合可無衝突合併,故安全自動執行。
4. **`hb tags`** —— 提供列出現有 tag 的指令,方便撰寫 frontmatter 前
   對照、降低 typo 生垃圾 tag。
...
2. **managed / 可編** —— `cardId`、`type`、`contentMd5`、`syncedAt`、
   `schemaVersion` 由 hbedit 管理,使用者不應手改;`tags` 可編、雙向
...
- **§2 / `v1/skill/scripts/hbedit.py`** —— `_vault_root` 從「往上找 `notes/` 父層」改為
  「往上找含 `.hbedit/` 的目錄」(§8.2)。
...
- **§4 / `v1/skill/scripts/hbedit.py`** —— 現行 push 以重新 `note read` 的當下 md5 當
  樂觀鎖,等於鎖失效、永不偵測衝突。2026-05-22 dogfood 撞車實測證實:
  本地與遠端在同一同步點後各改一行,`hb push` 回報 `updated` 成功、
...
- **`v1/skill/scripts/pm2md.py`** —— 連續的 `numbered_list_item` 轉回 markdown 時每項
  都印 `1.` 而非遞增(2026-05-22 dogfood `hb push` 一張白話設計卡時發現)。
  卡片在 Heptabase 上顯示正常(Heptabase 數連續兄弟節點自動編號),但
  `hb pull` 下來的 `.md` 會是 `1. 1. 1.`,在 round-trip 製造假 diff、
  使 `hb sync` 的 hash-based dirty 判斷誤判。修法:轉換時對連續
  `numbered_list_item` 維護遞增計數器。連帶 §4 的「E02 無損」應釐清為
  「結構無損,但非 byte 無損」—— 而 `hb sync` 的 dirty 判斷正是靠 hash,
  故此類非 byte 無損的轉換差異都須在 pull 端消除。
- **`v1/skill/scripts/hbedit.py` `pull`(檔案身分)** —— `pull` 一律以 `_slug(title)` 推導
...
- **`v1/skill/scripts/hbedit.py` tag 同步(雙向皆未實作)** —— `pull` 呼叫 `build_note_meta`
...
> 本章定義 `hb` / 決策合約 / agent 的三層分工,以及如何同時支援
> Claude Code、Codex、opencode 等多套 agent 工具。它回答的是「§8 的
> 行為由誰執行」,不改變 §8 的行為本身。

### 9.1 三層分工

- **`hb`(程式)= 偵測 + 回報。** 只做機械性操作;遇到任何需要判斷的
  情況(衝突、超大、tag 疑似 typo),**不自行決策**,而是以結構化方式
  回報。
- **決策合約 = 決策層。** 窮舉規範「`hb` 回報 X → agent 做 Y」。
  tool-agnostic、單一來源(見 §9.3)。
- **agent(Claude Code / Codex / opencode)= 執行者。** 讀 `hb` 回報,
  依合約執行,包括把使用者選定的 resolution 再交回 `hb`。

§8 的行為(衝突→備份+重 pull、超大→建議切卡、批次→先確認)在此架構
下不變,只是明確:`hb` 偵測並回報、agent 依合約執行。「自動」對使用者
而言不變,差別在動作由 agent 跑、而非 `hb` 內部跑。

### 9.2 `hb` 輸出作為 contract

agent 既然依 `hb` 輸出做決策,該輸出即 API contract,要求:
...
- 偵測在 `hb`、決策在合約、**執行再回到 `hb`**:每條 resolution 都要有
  對應指令(例如 `oversized` 的切卡 → `hb split`)。
...
v1/skill/                 ← 這個目錄就是產品:直接 commit、直接開發、直接安裝
  ...
  scripts/                ← Anthropic skill 慣例的程式目錄(非 lib/)
    hb                    ← 可執行 shim(chmod +x):realpath 自我定位後 import hbedit
    hbedit.py  frontmatter.py  vault.py  tagsync.py  pm2md.py  transplant.py  htb.py
```
...
- **無 build、無重複檔** —— `v1/skill/` 在 repo 裡就是出貨形狀,開發直接
  改 `scripts/` 裡的檔。`hbedit.py` 的 import 只需 `sys.path.insert(0, _HERE)`
...
- **`hb` 上 PATH** —— `SKILL.md` 全程只寫 `hb ...`,不用工具特定路徑變數
  (`${CLAUDE_SKILL_DIR}` 僅 Claude Code 有)。安裝步驟把 `scripts/hb`
  symlink 到 PATH;`scripts/hb` 以 `os.path.realpath(__file__)` 自我定位,
...
同步前必須確認 Heptabase CLI 可用。此檢查**做進 `hb` 自己**(指令
`hb doctor`),不靠 `SKILL.md` 散文自覺、也不靠 Claude Code 專屬的 `!`
注入 —— 才能三平台一致可攜,並貼合 §9.1「`hb` 偵測、agent 依合約決策」。

`hb doctor` 三項檢查,輸出 §9.2 的結構化 status:
...
`SKILL.md`(三平台共用)規範一句:**動任何同步前先跑 `hb doctor`;非
`ok` 就停下、照其 `detail` 訊息告知使用者**。`hb pull` / `hb push`
